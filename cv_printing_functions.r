...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$it_skills   <- read_gsheet(sheet_id = "it_skills")
    cv$contact_info  <- read_gsheet(sheet_id = "contact_info")
    cv$text_blocks  <- read_gsheet(sheet_id = "text_blocks")
  } else {
    # Want to go old-school with csvs?
    cv$entries_data <- readr::read_csv(paste0(data_location, "entries.csv"), skip = 1)
    cv$lang_skills       <- readr::read_csv(paste0(data_location, "language_skills.csv"), skip = 1)
    cv$it_skills  <- readr::read_csv(paste0(data_location, "it_skills.csv"), skip = 1)
    cv$contact_info <- readr::read_csv(paste0(data_location, "contact_info.csv"), skip = 1)
    cv$contact_info <- readr::read_csv(paste0(data_location, "text_blocks.csv"), skip = 1)
  }
...
print_lang_skill_bars <- function(cv, out_of = 10, bar_color = "#468499", bar_background = "#c0d6e4", glue_template = "default"){
  # #808080 #A9A9A9
  if(glue_template == "default"){
    glue_template <- "
<div
...
#' @description Construct a bar chart of skills
#' @param out_of The relative maximum for skills. Used to set what a fully filled in skill bar is.
print_it_skill_bars <- function(cv, out_of = 10, bar_color = "#778899", bar_background = "#B0C4DE", glue_template = "default"){
  # #969696  #d9d9d9
  if(glue_template == "default"){
    glue_template <- "
<div
...
print_contact_info <- function(cv){
  glue::glue_data(
    cv$contact_info,
    "<i class='fa-{icon_class} fa-{icon}' style='color:steelblue'></i>&nbsp;|-&nbsp; [{contact}]({pfad}){{.external target='_blank'}}<br>"
    #[{contact}]({link}){.external target='_blank'}
  ) %>% print()
